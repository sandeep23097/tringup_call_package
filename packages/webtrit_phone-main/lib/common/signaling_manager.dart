...
class SignalingManager {
  ...
  final void Function(MissedCallEvent)? onMissedCall;
  ...
  static const _kMaxBackgroundReconnectAttempts = 5;
  // Backoff delays in seconds: 2, 5, 10, 20, 30
  static const _kBackoffSeconds = [2, 5, 10, 20, 30];
  ...
  void _handleDisconnect(int? code, String? reason) {
    _logger.warning('Disconnected from signaling server: code=$code reason=$reason');
    _isConnected = false;
    onDisconnect?.call(code, reason);

    if (!enableReconnect) return;

    if (_backgroundReconnectAttempts >= _kMaxBackgroundReconnectAttempts) {
      _logger.severe('Background signaling: max reconnect attempts ($_kMaxBackgroundReconnectAttempts) reached — giving up');
      _backgroundReconnectAttempts = 0;
      return;
    }

    final delaySecs = _kBackoffSeconds[_backgroundReconnectAttempts.clamp(0, _kBackoffSeconds.length - 1)];
    final delay = Duration(seconds: delaySecs);
    _logger.info('Background signaling: scheduling reconnect in $delay (attempt $_backgroundReconnectAttempts)');

    _backgroundReconnectTimer?.cancel();
    _backgroundReconnectTimer = Timer(delay, () async {
      final hasNet = await hasNetworkConnection();
      if (!hasNet) {
        _logger.warning('Background signaling: no network — skipping reconnect attempt $_backgroundReconnectAttempts');
        return;
      }
