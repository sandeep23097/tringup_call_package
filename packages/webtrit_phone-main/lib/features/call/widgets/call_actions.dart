...
class CallActions extends StatefulWidget {
  ...
  final VoidCallback? onAddParticipantPressed;
    ...
    if (widget.isIncoming && !widget.wasAccepted) {
      ...
      if (_keypadShow) {
        ...
        actions = [
          ...
          if (widget.groupCallEnabled) ...[
            Tooltip(
              message: 'Add participant',
              child: TextButton(
                onPressed: widget.enableInteractions ? widget.onAddParticipantPressed : null,
                style: widget.style?.key,
                child: Icon(Icons.group_add, size: actionPadIconSize),
              ),
            ),
            const SizedBox(),
            const SizedBox(),
